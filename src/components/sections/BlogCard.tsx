...
const BlogCard = ({ post, idx = 0 }: { post: Post; idx?: number }) => (
  <Link
    to={`/blog/${post.slug}`}
    className="group flex flex-col h-full rounded-[2rem] border border-white/10 bg-white/5 overflow-hidden transition-all hover:border-[hsl(var(--gold-soft))/0.3] hover:bg-white/[0.08]"
  >
    {post.cover ? (
    ...
    <div className="flex flex-col flex-1 p-8">
      <div className="flex items-center gap-3 text-[10px] font-bold uppercase tracking-widest text-white/30 mb-4">
        <span>{post.date}</span>
      ...
      <p className="mt-4 line-clamp-2 text-sm text-white/50 leading-relaxed font-medium">
        {post.excerpt}
      </p>
